...
void *thread_func(void* argp)
{
	...
	while(isrunning)
	{
		...
		err = mq_receive(*msg_q_status, (char*) (&status), sizeof(int), &prio);
		if(err < 0)
		{   
		  if (errno == EAGAIN)
		     printf("Cmd Message queue empty, skiping read\n");
		  else if (errno == EBADF)
		    printf("Failed to read s to msg q becauuse of bad fd\n");
		  else if (errno == EMSGSIZE)
		    printf("Failed to readsetopints to msg q \n");
...
int main()
{
	...
	mqd_t msg_q_status;
	msg_q_status = mq_open("/dev/rtp3", O_RDONLY | O_NONBLOCK | O_CREAT, 0666, &status_q_attr);
  	if(msg_q_status < 0)
   	printf("Failed to open data msg q\n");
	int err;
	spi_command_t cmd;
